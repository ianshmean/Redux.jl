...
function renderloop(window, ctx)
    try
        while !GLFW.WindowShouldClose(window)
            ...
            boolref = Ref(false)
            CImGui.Checkbox("Unchecked Checkbox", boolref)
...
function render(;width=360, height=480, title::AbstractString="Basic Window")
    window, ctx = init_renderer(width, height, title)
    GC.@preserve window ctx begin
        renderloop(window, ctx)
    end
end
